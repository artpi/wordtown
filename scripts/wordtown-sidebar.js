...
const WordTownSidebar = () => {
		...
		apiFetch( {
			...
			.then( ( response ) => {
				...
				if ( response.success && response.wordtown_tile_scheduled ) {
					...
					setGenerationStatus(
						`Tile generation scheduled (Job ID: ${ response.wordtown_tile_scheduled })`
					);
				} else {
					setGenerationStatus(
			...
			.catch( ( err ) => {
				...
				setGenerationStatus(
					`Error: ${
						err.message || 'Failed to schedule tile generation'
					}`
				);
				setIsGenerating( false );
			} );
	...
	const renderTileContent = () => {
		...
		if ( ! tileData ) {
			return createElement(
				...
				createElement(
					'p',
					{},
					'Tile ID found but media data could not be loaded.'
				)
			);
		}
		...
		return createElement(
			'div',
			{ className: 'wordtown-tile-display' },
	...
	const renderGenerationPanel = () => {
		return createElement(
			'div',
			{ className: 'wordtown-generation-panel' },
			scheduled || generationStatus
				? createElement(
						'div',
						{ className: 'wordtown-scheduled-info' },
						createElement( 'p', {}, 'Tile generation scheduled:' ),
						createElement(
							'p',
							{ className: 'wordtown-scheduled-time' },
							generationStatus || `Job ID: ${ scheduled }`
						)
				  )
				: isGenerating
				? createElement(
						'div',
						{ className: 'wordtown-generating' },
						createElement( Spinner ),
						createElement(
							'p',
							{},
							'Requesting tile generation...'
						)
				  )
				: tileId
				? createElement(
