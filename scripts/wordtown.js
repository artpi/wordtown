...
function initWordTown() {
	...
	instructions.textContent =
		'Scroll to navigate | Mouse Wheel + Ctrl: Zoom | Click: Open Tile';
...
function setupEventListeners( container, gridContainer ) {
	...
	container.addEventListener( 'wheel', ( e ) => {
		...
		if ( e.ctrlKey ) {
			...
			const zoomDelta =
				e.deltaY > 0 ? -config.zoomSpeed : config.zoomSpeed;
...
function calculateInitialZoom( container, gridContainer ) {
	...
	currentZoom = Math.max(
		config.minZoom,
		Math.min( config.maxZoom, fitZoom )
	);
	...
	console.log(
		`Initial zoom set to ${ currentZoom } (container: ${ containerWidth }x${ containerHeight }, grid: ${ gridWidth }x${ gridHeight })`
	);
}
...
function centerView( container ) {
	...
	const scrollLeft =
		( gridBounds.minX + gridWidth / 2 ) * currentZoom -
		container.clientWidth / 2;
	const scrollTop =
		( gridBounds.minY + gridHeight / 2 ) * currentZoom -
